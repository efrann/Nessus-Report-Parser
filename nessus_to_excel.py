...
ROW_TRACKER = dict()
# Child Elements
SINGLE_FIELDS = ['risk_factor', 'description',
...
def generate_worksheets():  # pylint: disable=too-many-statements, too-many-branches, line-too-long
    ...
    for sheet in ws_names:
        ...
        if sheet == "Graphs":
            continue
        if sheet == "Overview":
            active_ws.set_column('A:A', 30)
            active_ws.set_column('B:B', 90)
        ...
        if sheet == "Full Report":
            ...
            active_ws.write(1, 0, 'IP Address', CENTER_BORDER_FORMAT)
            active_ws.write(1, 1, 'Port', CENTER_BORDER_FORMAT)
          ...
          # active_ws.write(1, 10, 'Plugin Family', CENTER_BORDER_FORMAT)
            active_ws.write(1, 2, 'Plugin Name', CENTER_BORDER_FORMAT)
            active_ws.write(1, 3, 'Description', CENTER_BORDER_FORMAT)
            active_ws.write(1, 4, 'Synopsis', CENTER_BORDER_FORMAT)
            active_ws.write(1, 5, 'Plugin Output', CENTER_BORDER_FORMAT)
            active_ws.write(1, 6, 'Solution', CENTER_BORDER_FORMAT)
            active_ws.write(1, 7, 'Exploit Available', CENTER_BORDER_FORMAT)
          ...
          #                  CENTER_BORDER_FORMAT)
            ...
            active_ws.autofilter('A2:I2')
            active_ws.set_column('A:A', 14)
            active_ws.set_column('B:B', 8)
            active_ws.set_column('C:C', 15)
            active_ws.set_column('D:D', 20)
            active_ws.set_column('E:E', 20)
            active_ws.set_column('F:F', 20)
            active_ws.set_column('G:G', 14)
            active_ws.set_column('H:H', 25)
            active_ws.set_column('I:I', 20)
      ...
      #     continue
        ...
        active_ws.merge_range('F1:J1', 'Manuel Kontrol', CENTER_MANUAL_FORMAT)    
        ...
        active_ws.write(1, 2, 'Plugin Name', CENTER_BORDER_FORMAT)
        active_ws.write(1, 3, 'Exploit Avaiable', CENTER_BORDER_FORMAT)
        #active_ws.write(1, 8, 'Exploit by Malware', CENTER_BORDER_FORMAT)
        active_ws.write(1, 4, 'CVE Information', CENTER_BORDER_FORMAT)
        active_ws.write(1, 5, 'PoC Durumu', CENTER_BORDER_FORMAT)
        active_ws.write(1, 6, 'Kontrol Eden Kisi', CENTER_BORDER_FORMAT)
        active_ws.write(1, 7, 'Onerilen Cozum', CENTER_BORDER_FORMAT)
        active_ws.write(1, 8, 'Son Kontrol Tarihi', CENTER_BORDER_FORMAT)
        active_ws.write(1, 9, 'Son Durumu', CENTER_BORDER_FORMAT)
        
        #active_ws.write(1, 10, 'Bugtraq ID Information', CENTER_BORDER_FORMAT)

        active_ws.freeze_panes('C3')
        active_ws.autofilter('A2:J2')
        active_ws.set_column('A:A', 15)
        active_ws.set_column('B:B', 15)
        active_ws.set_column('C:C', 20)
        active_ws.set_column('D:D', 30)
        active_ws.set_column('E:E', 25)
        active_ws.set_column('F:F', 20)
        active_ws.set_column('G:G', 25)
        active_ws.set_column('H:H', 20)
        active_ws.set_column('I:I', 25)
        active_ws.set_column('J:J', 20)
        #active_ws.set_column('K:K', 25)
...
def add_overview_data(sev, seen_ip):
    """
        Generating overview
    """
    ColorPrint.print_warn("\nOzet Rapor Olusturuluyor")
    active_ws = WS_MAPPER['Overview']

    active_ws.write(2, 0, "Taranan Toplam IP Sayisi", LIGHT_FORMAT)
    active_ws.write(2, 1, seen_ip, NUMBER_FORMAT)

    active_ws.write(3, 0, "Taranan Farkli IP Sayisi", LIGHT_FORMAT)
    active_ws.write(3, 1, len(UNIQUE_IP_LIST), NUMBER_FORMAT)

    active_ws.write(4, 0, "", SM_DARK_FORMAT)
    active_ws.write(4, 1, "", SM_DARK_FORMAT)

    active_ws.write(5, 0, "Farkli Critical Zafiyetler", LIGHT_FORMAT)
    active_ws.write(5, 1, len(COMMON_CRIT), NUMBER_FORMAT)

    active_ws.write(6, 0, "Farkli High Zafiyetler", LIGHT_FORMAT)
    active_ws.write(6, 1, len(COMMON_HIGH), NUMBER_FORMAT)

    active_ws.write(7, 0, "Farkli Medium Zafiyetler", LIGHT_FORMAT)
    active_ws.write(7, 1, len(COMMON_MED), NUMBER_FORMAT)

    active_ws.write(8, 0, "Farkli Low Zafiyetler", LIGHT_FORMAT)
    ...
    active_ws.write(9, 0, "", SM_DARK_FORMAT)
    active_ws.write(9, 1, "", SM_DARK_FORMAT)

    active_ws.write(10, 0, "Toplam Critical Zafiyetler", LIGHT_FORMAT)
    active_ws.write(10, 1, sev["Critical"], NUMBER_FORMAT)

    active_ws.write(11, 0, "Toplam High Zafiyetler", LIGHT_FORMAT)
    active_ws.write(11, 1, sev["High"], NUMBER_FORMAT)

    active_ws.write(12, 0, "Toplam Medium Zafiyetler", LIGHT_FORMAT)
    active_ws.write(12, 1, sev["Medium"], NUMBER_FORMAT)

    active_ws.write(13, 0, "Toplam Low Zafiyetler", LIGHT_FORMAT)
    active_ws.write(13, 1, sev["Low"], NUMBER_FORMAT)

    #active_ws.write(15, 0, "Toplam Informational Zafiyetler", LIGHT_FORMAT)
    #active_ws.write(15, 1, sev["Informational"], NUMBER_FORMAT)

    active_ws.write(14, 0, "", SM_DARK_FORMAT)
    active_ws.write(14, 1, "", SM_DARK_FORMAT)

######## Critical Vulnerabilities ##########
    active_ws.write(15, 0, "Farkli Critical Zafiyetler", SM_RED_FORMAT)
    if COMMON_CRIT:
        top_crit = sorted(COMMON_CRIT, key=lambda key:
                          COMMON_CRIT[key], reverse=True)
        for crit in top_crit:
            active_ws.write(15 + top_crit.index(crit), 1, crit, RED_TEXT_FORMAT)


#Critical and High section seperator. You can change the starting point.
#In this case it depends on the amount of the critica vulnerability
#17 + sev [] = seperator   
            
    active_ws.write(15 + len(top_crit), 0, "", SM_RED_FORMAT)
    active_ws.write(15 + len(top_crit), 1, "", SM_RED_FORMAT)

######## High Vulnerabilities ##########
# Starts from number 17crits (22th to the number of found critical vulns.) 

    active_ws.write(15, 0, "Farkli High Zafiyetler", SM_REDLIGHT_FORMAT)
    if COMMON_HIGH:
        top_high = sorted(COMMON_HIGH, key=lambda key:
                          COMMON_HIGH[key], reverse=True)
        for high in top_high:
            active_ws.write(15 + len(top_crit) + 1 + top_high.index(high),
                            1,
                            high,
                            REDLIGHT_TEXT_FORMAT)
    active_ws.write(15 + len(top_crit) + 1 + len(top_high), 0, "", REDLIGHT_TEXT_FORMAT)
    active_ws.write(15 + len(top_crit) + 1 + len(top_high), 1, "", REDLIGHT_TEXT_FORMAT)                

def add_chart_data(data):
    """
        Generation of graphs
    """
    ColorPrint.print_warn("\nSeviyelerine Gore Zafiyet Grafigi Olusturuluyor")
    ...
    # If you want to see he Informational vulns on the graph 
    # you should change the 5 to 6
    severity_chart.set_size({'width': 624, 'height': 480})
    severity_chart.add_series({
        'name': 'Total Vulnerabilities',
        'data_labels': {'value': 1},
        'categories': ["Graph Data", 2, 0, 5, 0],
        'values': ["Graph Data", 2, 1, 5, 1],
        'points': [
            {'fill': {'color': '#58BF65'}}, # green - Low
            {'fill': {'color': '#618ECD'}}, # blue - Medim
            {'fill': {'color': '#E9A23A'}}, # orange - High
            {'fill': {'color': '#922B21'}}, # red - Critical
        ]
    })
    severity_chart.set_title({'name': 'Seviyelerine Gore Zafiyetler'})
...
def add_report_data(report_data_list, the_file):
    """
        ...
        worksheet
    """
    ColorPrint.print_bold("\tFull Rapor Sayfasi Olusturuluyor")
    ...
    for reportitem in report_data_list:
      ...
      #                     reportitem["vuln_publication_date"], NUMBER_FORMAT)
        ...
        report_ws.write(temp_cnt, 0, reportitem['host-ip'], WRAP_TEXT_FORMAT)
        report_ws.write(temp_cnt, 1, int(reportitem["port"]), NUMBER_FORMAT)
        report_ws.write(temp_cnt, 2, reportitem["pluginName"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 3, reportitem["description"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 4, reportitem["synopsis"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 5, reportitem["plugin_output"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 6, reportitem["solution"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 7, reportitem["exploit_available"], NUMBER_FORMAT)
        report_ws.write(temp_cnt, 8, reportitem["cve"], NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 4, reportitem['host-fqdn'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 5, reportitem[ "vuln_publication_date"], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 7, int(reportitem["severity"]), NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 8, reportitem[ "risk_factor"], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 9, int(reportitem["pluginID"]), NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 10, reportitem["pluginFamily"], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 11, reportitem[ "pluginName"], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 12, reportitem["description"], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 13, reportitem['synopsis'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 14, reportitem['plugin_output'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 15, reportitem[ 'solution'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 16, reportitem[ 'exploit_available'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 17, reportitem[ 'exploitability_ease'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 18, reportitem['exploited_by_malware'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 19, reportitem['plugin_publication_date'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 20, reportitem['plugin_modification_date'], WRAP_TEXT_FORMAT)
      # report_ws.write(temp_cnt, 21, reportitem[ 'cve'], NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 22, reportitem['bid'], NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 23, reportitem['cvss_base_score'], NUMBER_FORMAT)
      # report_ws.write(temp_cnt, 24, reportitem['cvss_temporal_score'], NUMBER_FORMAT)
...
def add_vuln_info(vuln_list, the_file):
    ...
    for key, value in SEVERITIES.items():
        ColorPrint.print_bold(
            "\tVeriler {0} Sayfasina Isleniyor".format(value))
        ...
        for vuln in vuln_list:
            ...
            vuln_ws.write(temp_cnt, 0, vuln['host-ip'], WRAP_TEXT_FORMAT)
            vuln_ws.write(temp_cnt, 1, int(vuln['port']), NUMBER_FORMAT)
            ...
            vuln_ws.write(temp_cnt, 2, vuln['pluginName'], WRAP_TEXT_FORMAT)
            vuln_ws.write(temp_cnt, 3, vuln['exploit_available'], WRAP_TEXT_FORMAT)
            #vuln_ws.write(temp_cnt, 4, vuln['exploited_by_malware'], WRAP_TEXT_FORMAT)
            vuln_ws.write(temp_cnt, 4, vuln['cve'], WRAP_TEXT_FORMAT)
...
def begin_parsing():  # pylint: disable=c-extension-no-member
    ...
    for report in TO_BE_PARSED:
        ...
        if root.tag in ["NessusClientData_v2"]:
            ColorPrint.print_pass(
                "\n {0} dosyasinin uzerinde calisiliyor".format(report))
        ...
        del context
        curr_iteration += 1
        ColorPrint.print_bold("\n{0}% ({1}/{2}) tamamlanan rapor kismi".format(
...
if __name__ == "__main__":
    ...
    CENTER_MANUAL_FORMAT = WB.add_format(
        {'bg_color' : '#5499C7',
         'font_color' : 'white',
         'font_size' : 12,
         'align' : 'center',
    ...
    RED_TEXT_FORMAT = WB.add_format(
        {'font_color':'#E9A23A',
         'bold':1,
         'border':True})    
    
    SM_REDLIGHT_FORMAT = WB.add_format(
        {'bg_color': '#E9A23A',
         'font_color': 'white',
         'font_size': 12,
         'border': 1,
    ...
    REDLIGHT_TEXT_FORMAT = WB.add_format(
        {'font_color':'#E9A23A',
         'bold':1,
         'border':True})        
    ...
    ColorPrint.print_warn(
        "\n*** Max beklenen memory kullanimi: {0} MB ***".format(MAX_EXPECTED_MEMORY_USAGE))
    ...
    ColorPrint.print_pass(
        "\nRaporunuz {0}.xlsx adiyla kaydedildi.".format(REPORT_NAME))
